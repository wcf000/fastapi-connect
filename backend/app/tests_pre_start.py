...
def init_supabase() -> None:
    """Initialize and check Supabase connection for tests"""
    try:
        ...
        logger.info("Verifying Supabase connection for tests...")
        ...
        logger.error(f"Error connecting to Supabase for tests: {e}")
        raise e
...
def main() -> None:
    ...
    if supabase_url.strip() and supabase_key.strip():
        logger.info("Using Supabase backend for tests")
        init_supabase()
    else:
        logger.info("Using PostgreSQL backend for tests")
        init(engine)
